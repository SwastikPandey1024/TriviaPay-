...
export async function makePaymentTxn(from, to, amountAlgo, note = '') {
  if (!from || !to) {
    throw new Error('Address must not be null or undefined. Ensure both sender and receiver are valid Algorand addresses before creating transaction.')
  }
  const algod       = getAlgodClient()
...
export async function submitSignedTxn(signedTxnBlob) {
  const algod  = getAlgodClient()
  // Accept several signed formats from wallets/connectors:
  // - Uint8Array / ArrayBuffer
  // - base64 string
  // - object { signedTransaction: base64 }
  // Normalize to Uint8Array
  let rawBlob = signedTxnBlob
  // If an array (e.g. [signed1, signed2]) assume first element is intended
  if (Array.isArray(rawBlob) && rawBlob.length > 0) rawBlob = rawBlob[0]
  // If object with signedTransaction property (Pera returns this shape sometimes)
  if (rawBlob && typeof rawBlob === 'object' && rawBlob.signedTransaction) rawBlob = rawBlob.signedTransaction
  // If base64 string, convert to Uint8Array
  if (typeof rawBlob === 'string') {
    try {
      const binStr = atob(rawBlob)
      const bytes = new Uint8Array(binStr.length)
      for (let i = 0; i < binStr.length; i++) bytes[i] = binStr.charCodeAt(i)
      rawBlob = bytes
    } catch (e) {
      // fallthrough — let algosdk/algod attempt to handle it and throw clearer error
    }
  }

  const result = await algod.sendRawTransaction(rawBlob).do()
