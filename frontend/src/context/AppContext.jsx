...
function appReducer(state, action) {
  switch (action.type) {
    ...
    case ACTIONS.LOGIN_USER: {
      ...
      const walletAddress = user?.walletAddress ?? state.address
      return {
        ...state,
        user,
    ...
    case ACTIONS.SET_AUTH_RESTORED:
      return {
        ...state,
        authRestored: action.payload,
      }
...
export function AppProvider({ children }) {
  ...
  const refreshAll = useCallback(async (addressOverride) => {
    ...
    // Development/demo shortcut: if using a placeholder demo address, don't call external APIs.
    // The demo address `DEMOADDRESS...` is not a valid Algorand address and causes 400 errors
    // from Algorand REST endpoints. Short-circuit and provide mock data for local development.
    if (typeof addr === 'string' && addr.startsWith('DEMOADDRESS')) {
      // Mock dynamic data for local development
      const now = new Date()
      const today = now.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })

      // Create a set of mock transactions over the last 3 days
      const nowTs = Date.now()
      const oneDay = 24 * 60 * 60 * 1000
      const mockTxs = [
        // Yesterday: someone requested a split (0-ALGO notification tx)
        {
          id: 'tx_demo_notify_1',
          type: 'Payment',
          amount: '0.0000',
          sender: 'Friend…001',
          senderFull: 'SB2RNDY3THY7EXAMPLEADDRESSFORFRIEND001',
          receiver: 'DEMO…ADDR',
          receiverFull: addr,
          rawNote: NOTIFY_PREFIX + JSON.stringify({ billId: 'bill_123', billName: 'Pizza Night', billNote: 'Bring cash', share: 0.75, total: 3, creatorAddress: 'SB2RNDY3THY7EXAMPLEADDRESSFORFRIEND001', date: new Date(nowTs - oneDay).toISOString() }),
          status: 'success',
          date: new Date(nowTs - oneDay).toLocaleDateString('en-US'),
        },
        // Two days ago: payer sent partial payment
        {
          id: 'tx_demo_pay_1',
          type: 'Payment',
          amount: '2.5000',
          sender: 'ALGO01…AAAA',
          senderFull: 'ER745AB7H64MC7RO5PEL7YCDQ245JOHVPHN5WHO3FCGPI5Y7GHL5QGAT64',
          receiver: 'DEMO…ADDR',
          receiverFull: addr,
          rawNote: 'Lunch split share',
          status: 'success',
          date: new Date(nowTs - 2 * oneDay).toLocaleDateString('en-US'),
        },
        // Today: app interaction
        {
          id: 'tx_demo_app_1',
          type: 'App Call',
          amount: '0.0000',
          sender: 'DEMO…ADDR',
          senderFull: addr,
          receiver: 'App #755792571',
          receiverFull: String(appId || '755792571'),
          rawNote: '',
          status: 'success',
          date: new Date(nowTs).toLocaleDateString('en-US'),
        },
        // Another small payment today
        {
          ...
          amount: '0.7500',
          sender: 'Friend…002',
          senderFull: 'SB2RNDY3THY7EXAMPLEADDRESSFORFRIEND002',
          receiver: 'DEMO…ADDR',
          receiverFull: addr,
          rawNote: 'Coffee share',
          status: 'success',
          date: new Date(nowTs).toLocaleDateString('en-US'),
        },
      ]
      ...
      const mockGlobal = [
        { key: 'owner', value: 'ER745AB7H64MC7RO5P' },
        { key: 'total', value: 3250000 },
      ]

      // Mock goals and bills to drive UI
      const mockGoals = [
        { id: 'goal1', name: 'Textbook Fund', target: 50, current: 12 },
        { id: 'goal2', name: 'Trip', target: 200, current: 40 },
      ]
      ...
      const mockBills = [
        {
          ...
          name: 'Pizza Night',
          total: 3,
          note: 'Split between friends',
          creator: 'SB2RNDY3THY7EXAMPLEADDRESSFORFRIEND001',
          payees: [
            { id: 'p1', address: addr, name: 'You', status: 'pending', share: 0.75 },
            { id: 'p2', address: 'SB2RNDY3THY7EXAMPLEADDRESSFORFRIEND001', name: 'Alice', status: 'paid', share: 2.25 },
          ],
          date: new Date(nowTs - oneDay).toISOString(),
        },
      ]

      dispatch({ type: ACTIONS.SET_BALANCE, payload: 3.25 })
      dispatch({ type: ACTIONS.SET_ESCROW_BALANCE, payload: 1.5 })
      dispatch({ type: ACTIONS.SET_RECENT_TXS, payload: mockTxs })
      dispatch({ type: ACTIONS.SET_APP_GLOBAL_STATE, payload: mockGlobal })
      dispatch({ type: ACTIONS.SET_LAST_REFRESHED, payload: new Date() })
      dispatch({ type: ACTIONS.SET_LOADING, payload: false })

      // Populate local app state with goals and bills so UI shows dynamic content
      mockGoals.forEach(g => dispatch({ type: ACTIONS.ADD_GOAL, payload: g }))
      mockBills.forEach(b => dispatch({ type: ACTIONS.ADD_BILL, payload: b }))

      // Add a sample notification (mirrors on-chain note parsing logic)
      try {
        const payload = { billId: 'bill_123', billName: 'Pizza Night', billNote: 'Bring cash', payeeName: 'You', share: 0.75, total: 3, creatorAddress: 'SB2RNDY3THY7EXAMPLEADDRESSFORFRIEND001', date: new Date(nowTs - oneDay).toLocaleDateString('en-US') }
        dispatch({ type: ACTIONS.ADD_NOTIFICATION, payload: {
          id: 'notif_bill_1', type: 'bill_request', billId: payload.billId, billName: payload.billName,
          billNote: payload.billNote, payeeName: payload.payeeName, payeeAddress: addr, share: payload.share,
          total: payload.total, creatorAddress: payload.creatorAddress, date: payload.date, read: false, onChain: true,
        } })
      } catch (e) {
        // ignore
      }
      ...
      dispatch({ type: ACTIONS.UPDATE_STATS, payload: { poolBalance: 1.5, totalDeposited: 3.25, txCount: mockTxs.length, pendingBills: 1 } })
  ...
  // ── Restore user session + reconnect wallet session on page load ─────────
  useEffect(() => {
    const SESSION_KEY = 'trivia_pay_session'

    async function restore() {
      dispatch({ type: ACTIONS.SET_AUTH_RESTORED, payload: false })

      // Restore user session from localStorage
      try {
        const saved = JSON.parse(localStorage.getItem(SESSION_KEY) ?? 'null')
        if (saved?.user) {
          dispatch({ type: ACTIONS.LOGIN_USER, payload: saved.user })
          if (saved.user.walletAddress) {
            dispatch({ type: ACTIONS.CONNECT_WALLET, payload: saved.user.walletAddress })
          }
        }
      } catch (err) {
        console.warn('[AppContext] failed to parse saved session', err)
      }

      // Attempt Pera reconnect for wallet session
      try {
        ...
        dispatch({ type: ACTIONS.SET_AUTH_RESTORED, payload: true })
      }
    }
  ...
  // ── Persist login session for reloads ───────────────────────────────────
  useEffect(() => {
    const SESSION_KEY = 'trivia_pay_session'
    if (state.user) {
