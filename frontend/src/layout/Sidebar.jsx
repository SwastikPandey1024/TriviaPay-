...
const NAV_ITEMS = [
  ...
  },
  {
    ...
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor"
        strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <path d="M3 7h18m-16 4h14m-12 4h10" />
...
function labelToPath(label) {
  // simple mapping from label to route path
  return (
    label === 'Dashboard' ? '/' :
    label === 'Deposit' ? '/deposit' :
    label === 'Bill Split' ? '/bill-split' :
    label === 'Payment Tracker' ? '/payment-tracker' :
    label === 'Goal Tracker' ? '/goal-tracker' :
    label === 'Transactions' ? '/transactions' :
    label === 'Scan & Pay' ? '/scan-pay' :
    label === 'Vendor Marketplace' ? '/vendor-marketplace' :
    label === 'Analytics' ? '/analytics' :
    label === 'Settings' ? '/settings' :
    label === 'Campus Wallet' ? '/campus-wallet' :
    label === 'AI' ? '/ai' :
    label === 'Login' ? '/login' :
    ...
    <>
      ...
      >
        ...
        <nav className="flex-1 px-3 space-y-1 overflow-y-auto">
          {(() => {
            // On auth pages we want minimal sidebar items.
            const authOnly = location.pathname === '/login' || location.pathname === '/register'
            let visibleItems = NAV_ITEMS
            if (authOnly) {
              visibleItems = NAV_ITEMS.filter(item => item.label === 'Login' || item.label === 'Register')
            } else if (!state.walletConnected) {
              visibleItems = NAV_ITEMS.filter(item => ['Dashboard', 'Login', 'Register'].includes(item.label))
            } else {
              visibleItems = NAV_ITEMS.filter(item => item.label !== 'Login' && item.label !== 'Register')
            }

            return visibleItems.map(({ label, icon }) => {
              if (label === 'Login' && state.walletConnected) {
                // Should never show login when connected.
                return null
              }

              const path = labelToPath(label)
              const isActive = location.pathname === path || state.activePage === label
